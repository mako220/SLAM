...
class SLAM(object):
  ...
  #video encoding
  def encoding(self):
    encode = subprocess.Popen(['ffmpeg', '-i', sys.argv[1],'-vcodec', 'h264_nvenc', '-preset', 'fast', '-qmin', '10', '-qmax', '52', 'video1.mp4'], stdout=subprocess.PIPE)
    print(encode.communicate()[0])
  #matches 2 frames
  def process_frame(self, img, pose=None, verts=None):
    ...
    if frame.id < 5 or True:
      ...
      # kinematic model
      #The kinematic model studies the motion of a robot mechanism regardless of forces and torque that cause it.
      #compute the position and orientation of robot relative to the base position.
    ...
    if len(self.mapp.points) > 0:
      ...
      # only the points that fit in the frame to be considered for projection
...
if __name__ == "__main__":
  ...
  while cap.isOpened():
    ...
    print("\n*** frame %d/%d ***" % (i, CNT))
    if ret == True:
      #process frame  executes main SLAM funnctions.
    ...
    if disp2d is not None:
      ...
      #paint function highlights the vectors and feature points
      disp2d.paint(img)
