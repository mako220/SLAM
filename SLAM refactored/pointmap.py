...
class Point(object):
  ...
  #add 1 after the array. converts to homogeneous coordinates
  def homogeneous(self):
    return add_ones(self.pt)
  #returns decriptors from key points and descriptor indexes
  def orb(self):
    return [f.des[idx] for f,idx in zip(self.frames, self.idxs)]
  #returns min hamming distance detected feature points
  def orb_distance(self, des):
    return min([hamming_distance(o, des) for o in self.orb()])
  #delets frames, generally those frames are passed whos feature points
  #cannot be matched in subsequent frames
  def delete(self):
    for f,idx in zip(self.frames, self.idxs):
      f.pts[idx] = None
    del self
  #adding those feature points to observation which cannot match in two
  #subsequent frames, to see if they can match with other frames in pipeline
...
class Map(object):
  #Map contains frames and points. Frames contain new pose and feature points
  #Already projected feature points are also contained in map
  def __init__(self):
    self.frames = []
    self.points = []
    self.max_frame = 0
  ...
  def optimize(self, local_window=LOCAL_WINDOW, fix_points=False, verbose=False, rounds=50):
    ...
    for p in self.points:
      ...
      #Eliminating points above certain error threshold
